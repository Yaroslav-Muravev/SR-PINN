...
# Файловый обработчик (всё, уровень DEBUG)
...
class CylinderStressDataset(Dataset):
    def __init__(self, data_dir: str, csv_df: pd.DataFrame, ids: List[int], mesh_type: str,
        ...
        for idx, row in self.df.iterrows():
            ...
            if subsample_ratio < 1.0:
                ...
                # Сортируем индексы для сохранения порядка (опционально)
                keep = np.sort(keep)
        ...
        if normalize and self.total_points > 0:
            ...
            if external_stats is not None:
                ...
                print(
                    f"Using external fields stats: mean[6]={self.fields_mean_np[6]:.3e}, std[6]={self.fields_std_np[6]:.3e}")
            else:
    ...
    def get_boundary_batch(self, id_: int):
        """Возвращает батч из всех точек на верхнем и нижнем торцах для указанного ID."""
        ...
        # Собираем данные для этих индексов
        ...
        # Маски для этих точек (оставляем булевы)
...
class ResidualBlock(nn.Module):
    def __init__(self, dim: int, dropout: float = 0.05):  # <-- добавили dropout параметр
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.act1 = nn.SiLU()
        self.dropout1 = nn.Dropout(p=dropout)              # <-- dropout после первой активации
        self.linear1 = nn.Linear(dim, dim)
        self.norm2 = nn.LayerNorm(dim)
        self.act2 = nn.SiLU()
        self.dropout2 = nn.Dropout(p=dropout)              # <-- dropout после второй активации
...
class SRPINN(nn.Module):
    def __init__(self,
                 ...
                 dropout: float = 0.20):                   # <-- прокидываем dropout в блоки
        super().__init__()
...
def train_srpinn(model, train_loader, val_dataset, colloc_loader, component_weights,
                 voltage_loader, n_epochs, device, lr=1e-5, pde_every=5, voltage_every=100):
    # EXPERIMENT 1: AdamW + scheduler
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        ...
        factor=0.5,
        patience=15
        # verbose=True – убран для совместимости
    )
    ...
    for epoch in range(n_epochs):
        ...
        # Обновление scheduler и вывод нового lr, если он изменился
        old_lr = optimizer.param_groups[0]['lr']
        scheduler.step(avg_val_loss)
        new_lr = optimizer.param_groups[0]['lr']
        if new_lr != old_lr:
            logging.info(f"Epoch {epoch}: ReduceLROnPlateau reduced LR to {new_lr:.2e}")
    ...
    # --- EXPERIMENT 2: L-BFGS refinement ---
    logging.info("Starting L-BFGS refinement stage...")
    ...
    def closure():
        ...
        for batch in train_loader:
            batch = to_device(batch, device)
            loss, _ = criterion(model, batch, None)
            total_loss += loss
        total_loss.backward()
        return total_loss
    ...
    for i in range(10):
        ...
        logging.info(f"L-BFGS Step {i} complete.")

    model.eval()
    final_voltage_error = compute_voltage_error(model, val_dataset, device, verbose=False)
    if final_voltage_error < best_voltage_error:
        best_voltage_error = final_voltage_error
        torch.save(model.state_dict(), 'best_srpinn_model_voltage.pth')
        logger.info(f"L-BFGS improved voltage error to {best_voltage_error:.4f}, model saved.")



def parse_idx_train(data_dir: str) -> Tuple[List[int], List[int], List[int]]:
    """
    Извлекает ID из имен файлов, перемешивает их и разделяет на 
    train (60%), validation (20%) и test (20%).
    """
    import random
    ...
    ids = list(set(int(pattern.search(f).group(1)) 
                   for f in directory_contents if pattern.search(f)))

    # Перемешиваем список. 
    # seed гарантирует, что "случайность" будет одинаковой при каждом запуске.
    random.seed(42) 
    random.shuffle(ids)

    n = len(ids)
    
    # Расчет индексов для сплита
    test_split = n // 5          # 20%
    val_split = 2 * n // 5       # еще 20% (итого 40%)

    test_ids = ids[:test_split]
    val_ids = ids[test_split:val_split]
...
if __name__ == "__main__":
    ...
    voltage_loader = DataLoader(voltage_dataset, batch_size=1,
                                shuffle=True)  # каждый элемент – батч всех граничных точек одного ID
    ...
    # Модель (с dropout=0.05 по умолчанию)
    model = SRPINN(n_field_vars=7, dropout=0.05)   # <-- явно задаём dropout
    model = torch.compile(model)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    model_ntk = SRPINN(n_field_vars=7).to(device)  # для NTK можно без dropout
    ...
    # Обучение с lr=1e-3 (как в инструкции)
